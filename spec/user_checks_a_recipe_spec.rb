...
feature "User checks a recipe's deliciousness", %(
  ...
  scenario "user submits a recipe name containing 'pickled beets'" do
    visit '/'
    fill_in 'Recipe Name', with: 'Supa hot pickled beets'
    click_button 'Submit'

    expect(page).to have_content('Supa hot pickled beets')
    expect(page).to have_content('is a delicious recipe!')
  end

  scenario "user submits a recipe name without 'pickled beets'" do
    visit '/'
    fill_in 'Recipe Name', with: 'Supa hot rickled beets'
    click_button 'Submit'

    expect(page).to have_content('Supa hot rickled beets')
